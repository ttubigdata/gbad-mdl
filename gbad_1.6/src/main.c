...
// main.c
//
// SUBDUE, versions 5.1.2 - GBAD, version 1.6
//---------------------------------------------------------------------------
...
int main(int argc, char **argv)
{
   ...
   else
   {
	 ...
	 //
	    ...
	    //        patterns" (-norm) functionality.
	    //
            if (!parameters->noAnomalyDetection)
               printf("Applying GBAD algorithms...\n");
            //
	    ...
	    //
	       ...
	       //
	       // GBAD-P:  For this algorithm, on the first iteration ONLY, we need
	       //          to set the normative pattern to the user-specified
	       //          normative pattern (default is the best substructure).
	       //
		  ...
		  if (mdlSubList != NULL)
		     ...
		     printf("Anomalous Instances:  NONE\n\n");
                  ...
                  newSubList = PrintMaxPartialSubstructureInstances(subList, parameters);
	       }
	    ...
	    //
	       ...
	       printf("\n\n");
            ...
            {                                    // Another iteration?
               ...
               {
	          //
		  // GBAD-P:  For this algorithm, multiple iterations will need
		  ...
		  if ((iteration == 1) && (parameters->prob))
		  {
		     printf("Compressing graph by best substructure %lu:\n",
		            parameters->norm);
                     PrintSub(normSubListNode->sub,parameters);
		     printf("\n");
                     CompressFinalGraphs(normSubListNode->sub, parameters, 
		                         iteration, FALSE);
		  } else
         ...
         // GBAD-P
         //
         // Need to store information regarding initial
         // best substructure, for use in future probabilistic
         // calculations
         //
