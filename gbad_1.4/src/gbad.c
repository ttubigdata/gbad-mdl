...
SubList* CreateSubstructureAndInstancesList(SubList *subList, Parameters *parameters)
{
   ...
   while (subListNode != NULL)
   {
      ...
      {
         if ((parameters->labelList->labels[subListNode->sub->definition->vertices[i].label].labelValue.stringLabel != NULL) &&
	     (subLabelString != NULL) &&
                                                 ...
                                                 Parameters *parameters)
{
   ...
   while (subListNode != NULL)
   {
      for (i=0;i<subListNode->sub->definition->numVertices;i++)
      {
	    ...
	    // not a good way to compare to floating type variables.  I am
   ...
   // possible best anomalies).nomalousValue
   //
   if (anomInstanceList != NULL)
   {
      bestSub->instances = AllocateInstanceList();
