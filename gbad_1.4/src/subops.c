...
Substructure *AllocateSub()
{
   ...
   //
   // GBAD-MPS: Initialize parent values.
   //
...
Instance *AllocateInstance(ULONG v, ULONG e)
{
   ...
   instance->used = FALSE;
   //
   // GBAD-MDL: Initialize anomalous totals.
   //
   ...
   if (v > 0) 
   {
      ...
      //
      // GBAD-MDL: Intialize anomalous vertices.
      //
   ...
   if (e > 0) 
   {
      ...
      //
      // GBAD-MDL: Intialize anomalous edges.
      //
...
//          newly added edge.
//
// PURPOSE: Returns TRUE if subInstance and instance match on the new
//          new edge (and vertices) that were added.  Initially, a simple
...
//          of the SUBDUE work is performed.  It should also be noted that while
//          the code is broken up into multiple if-then-else sections, it was
//          done this way to make the code easier to read and understand.
//
// REVISIONS:
//
// 12/12/07 Fixed logic surrounding first match (if statement).
//
//---------------------------------------------------------------------------
                     ...
                     double threshold, double *cost)
{
   ...
   //    - are the new vertex labels the same?
         ...
         (g1->vertices[newVertex1].label == g2->vertices[newVertex2].label))))
   {
      ...
      //
      if (inst2->newVertex != VERTEX_UNMAPPED)
      {
