...
int main(int argc, char **argv)
{
   ...
   else
   {
      ...
      //
      // GBAD-MPS: Save the threshold for later, and set the threshold (for now)
      //           to 0.0.
      //
      if (parameters->mps)
      {
         parameters->mpsThreshold = parameters->threshold;
	 ...
	 //
         ...
         else 
         {
            ...
            // GBAD-MDL
            //
	    // NOTE:  The SUBDUE -threshold parameter has been overridden
	    //        with the mdlThreshold setting when the MDL anomaly
	    //        detection method is specified (see below).
	    //
            if ((parameters->mdl) && (!parameters->prob) && (!parameters->mps) &&
	        (parameters->mdlThreshold > 0.0))
            {
               //
               // Evaluate all of the instances of the top sub (perhaps extend this to
               // the top 3 subs?), finding which one is the true best sub
               // (because, with a non-zero threshold, even the anomalous
               // instance could have generated the substructure).
               //
	    ...
	    //
            ...
            // GBAD-MPS
            //
	    // NOTE:  For a multiple-specified approach, the MPS approach should be
	    //        the first approach used, as it is looking for smaller
	    //        substructures.  If the MDL approach is also specified, it will 
	    //        by default get the inexact matching because of the threshold.
	    //        If the probabilistic approach is also specified, that will be
	    //        applied in subsequent iterations by the logic below that turns
	    //        off the other algorithms.
	    //
...
Parameters *GetParameters(int argc, char *argv[])
{
   ...
   while (i < (argc - 1))
   {
	 ...
	 parameters->threshold = doubleArg;   // overriding -threshold specification
      }
