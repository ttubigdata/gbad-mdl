...
// 11/30/09  Eberle     Fixed output from GBAD-MPS algorithm; added frequency
...
//
// NOTE:  Currently, this routine is only called from main.c IF the -norm
//        option is chosen AND it is the first iteration.
//******************************************************************************
void RemoveSimilarSubstructures(SubList *subList, Parameters *parameters)
{
   ...
   while (subIndex != NULL)
   {
      ...
      // (or the user has specified to keep everything (default), with a value 
      // of 1.0)
      //
