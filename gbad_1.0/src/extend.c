...
//
// SUBDUE, versions 5.1.2 - GBAD, version 1.0
//---------------------------------------------------------------------------
...
SubList *ExtendSub(Substructure *sub, Parameters *parameters)
{
   ...
   //
   // GBAD-P:  Added anomalous flag and parameters.
   //
   ...
   if (negGraph != NULL)
      ...
      // GBAD-P:  Added anomalous flag and parameters.
      //
      negInstanceList = ExtendInstances(sub->negInstances, negGraph, FALSE,
                                        parameters);
      //
...
//
// GBAD-P:  Added anomalous flag and parameters (for anomaly detection type).
//
      ...
      for (v = 0; v < instance->numVertices; v++) 
      {
         ...
         for (e = 0; e < vertex->numEdges; e++) 
         {
            ...
            if (! edge->used) 
            {
               ...
               //
               // GBAD-P:  If probabilistic approach, and the extension is from
               //          a normative pattern to a normative pattern, do not
               //          add it (i.e. a normative pattern going to a normative
               //          pattern is not considered anomalous).
               //
               // Note: The strncmp looks backwards because of the comparison
               //       to false, but that is the way it works (because of the
               //       value returned from that function).
               //
               // Note: Check for newVertex < numVertices is for that
               //       situation where there is not a new vertex, and thus
               //       the value is set to some large undefined number.
               //
               // Note: This code assumed that all labels were strings... I have
               //       added some logic to prevent an error when it is not,
               //       but may want to play it on the safe side and just ensure
               //       that all input labels are strings (it won't affect the
               //       the validity of the approach)
               //
//               if ((parameters->prob)  &&
//                   (newInstance->newVertex < newInstance->numVertices) &&
//                   (parameters->labelList->labels[graph->vertices[newInstance->vertices[newInstance->newVertex]].label].labelValue.stringLabel != NULL) &&
//                   (!strncmp(parameters->labelList->labels[graph->vertices[newInstance->vertices[newInstance->newVertex]].label].labelValue.stringLabel,"SUB_",4)) &&
//                   (parameters->labelList->labels[graph->vertices[instance->vertices[v]].label].labelValue.stringLabel != NULL) &&
//                   (!strncmp(parameters->labelList->labels[graph->vertices[instance->vertices[v]].label].labelValue.stringLabel,"SUB_",4)))
////////////////////////////////////////////////////////// WFE - FIX
               if ((parameters->prob)  &&
                   (newInstance->newVertex < newInstance->numVertices) &&
                   (parameters->labelList->labels[graph->vertices[newInstance->vertices[newInstance->newVertex]].label].labelValue.stringLabel != NULL) &&
                   (!strncmp(parameters->labelList->labels[graph->vertices[newInstance->vertices[newInstance->newVertex]].label].labelValue.stringLabel,
		             SUB_LABEL_STRING,5)) &&
                   (parameters->labelList->labels[graph->vertices[instance->vertices[v]].label].labelValue.stringLabel != NULL) &&
                   (!strncmp(parameters->labelList->labels[graph->vertices[instance->vertices[v]].label].labelValue.stringLabel,
		             SUB_LABEL_STRING,5)))
////////////////////////////////////////////////////////// WFE - FIX
               {
                  // skip adding this extension....
               } else {
      ...
      while (instanceListNode != NULL) 
      {
         if (instanceListNode->instance != NULL) 
         {
                ...
                ((parameters->prob) && (parameters->currentIteration > 1)))
            {
               ...
               //
	       // NOTE:  The additional check for the current iteration is
	       //        due to an issue with this logic on subsequent
	       //        iterations (for anomaly detection - i.e., gbad).
	       //        Since the intent of this code was to speed-up
	       //        performance, for GBAD, that would only be
	       //        relevant at the first iteration.
	       //
	       if ((threshold == 0.0) && (!parameters->mdl) && 
	           (parameters->currentIteration == 1))
               {
                  //
               ...
               //
	       // GBAD NOTE:  All of the code above, from the "if (threshold.."
	       //             to here is the baseline SUBDUE 5.2.1 logic.
	       //             GBAD was originally tested with that code commented
	       //             out and the code below put into its place (also,
	       //             the "counter" code above and below was also 
	       //             originally commented out too).
	       //
	       //if (GraphMatch(sub->definition, instanceGraph, labelList,
               //               thresholdLimit, & matchCost, NULL))
               //{
