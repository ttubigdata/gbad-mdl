   ...
   while (subListNode != NULL)
   {
      ...
      if (bestSub->instances != NULL)
      {
         ...
         while (instanceListNode != NULL)
         {
            ...
            refInstanceListNode->edgeListSize = refGraph->numEdges;
            refInstanceListNode->vertexListSize = refGraph->numVertices;
      ...
      ProcessInstancesForSub(bestSub, refInstanceList, finalInstanceList,
                             parameters);
   ...
   while (refInstanceListNode != NULL)
   {
      ...
      while (!empty)
      {
         ...
         while (instanceListNode != NULL)
         {
            ...
            {
               printf("\ninstance num: %lu\n", index2);
       	       PrintInstance(gInstance, parameters->posGraph, 
	                     parameters->labelList);
            }
                          ...
                          Parameters *parameters)
{
   ...
   if (refInstanceListNode != NULL)
   {
      ...
      {
         newRefGraph = ExtendRefGraph(refInstanceListNode->refGraph, bestSub,
                                      &refInstanceListNode->vertexListSize,
                                      &refInstanceListNode->edgeListSize, graph, parameters);
   ...
   for (i=startVertexIndex;i<graph->numVertices;i++)
   {
      ...
      for (j=0;j<v.numEdges;j++)
      {
             ...
             (VertexInSubList(bestSubList, &vertices[e.vertex2])))
         {
            ...
            //
            extInstance =
               CreateExtendedInstance(instance, i, v.edges[j], graph, FALSE);
            //
