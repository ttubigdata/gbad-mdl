...
int main(int argc, char **argv)
{
   ...
   while ((iteration <= parameters->iterations) && (!done))
   {
      ...
      {
         //
         // Remove any substructures that are too similar to the best
         // substructure
         //
         // NOTE:  This feature was added as part of the "multiple normative
         //        patterns" (-norm) functionality.  The issue was that we were
         //        getting too many substructures that were very similar to 
         //        each other, and had been discovered because they were
         //        arrived at by a different extension.  So, when the user
         //        wants a different normative pattern than the best one, we
         //        need to make sure it is a pattern that is different.
         //
         if (parameters->norm > 1)
            RemoveSimilarSubstructures(subList, parameters);
...
Parameters *GetParameters(int argc, char *argv[])
{
   ...
   while (i < (argc - 1))
   {
      ...
      else if (strcmp(argv[i], "-mdl") == 0) 
      {
         ...
         {
            fprintf(stderr, "%s: Information Theoretic (MDL) ", argv[0]);
            fprintf(stderr, "threshold must be greater than 0.0 and less ");
            fprintf(stderr, "than 1.0\n");
            exit(1);
         }
         parameters->mdl = TRUE;
      ...
      else if (strcmp(argv[i], "-prob") == 0) 
      {
         ...
         if (ulongArg < 2)
         {
            fprintf(stderr, "%s: you must include a value greater ", argv[0]);
            fprintf(stderr, "than 1 as a parameter to the probabilistic ");
            fprintf(stderr, "anomaly detection method.\n");
            exit(1);
         }
         parameters->prob = TRUE;
         parameters->iterations = ulongArg;  // overrides -iterations specification
         parameters->maxAnomalousScore = 0.999999;  // overrides default of MAX_DOUBLE
      }
      else if (strcmp(argv[i], "-mps") == 0) 
      {
         ...
         if ((argumentExists != 1) || (doubleArg <= (double) 0.0) || 
             (doubleArg >= (double) 1.0))
         {
            fprintf(stderr, "%s: Maximum Partial Substructure (MPS) ", argv[0]);
            fprintf(stderr, "threshold must be greater than 0.0 and less ");
            fprintf(stderr, "than 1.0\n");
            exit(1);
         }
      ...
      else if (strcmp(argv[i], "-maxAnomalousScore") == 0) 
      {
         ...
         {
            fprintf(stderr, "%s: maximum anomalous score must be ", argv[0]);
            fprintf(stderr, "greater than 0.0 and less than %lf\n", MAX_DOUBLE);
            exit(1);
         }
	 //
	 ...
	 //
         ...
         {
            fprintf(stderr, "%s: maximum anomalous score must be ",argv[0]);
            fprintf(stderr, "greater than 0.0 and less than 1.0\n");
            exit(1);
         }
      ...
      else if (strcmp(argv[i], "-minAnomalousScore") == 0) 
      {
         ...
         if ((argumentExists != 1) || (doubleArg < (double) 0.0) || 
             (doubleArg >= (double) MAX_DOUBLE))
         {
            fprintf(stderr, "%s: minimum anomalous score must be ", argv[0]);
            fprintf(stderr, "greater than or equal to 0.0 and less than ");
            fprintf(stderr, "%lf\n",MAX_DOUBLE);
            exit(1);
         }
...
void PrintParameters(Parameters *parameters)
{
   ...
   if (!parameters->noAnomalyDetection) {
      ...
      if (parameters->similarity) 
         printf("  Similarity Percentage.......... %lf\n",parameters->similarity);
   }
