                      ...
                      ULONG startPosVertex, ULONG startNegVertex)
{
   ...
   vertexListSize = &parameters->posGraphVertexListSize;
   edgeListSize = &parameters->posGraphEdgeListSize;
   ...
   while (ReadToken(token, graphFile, &lineNo) != 0)
   {
      if (strcmp(token, POS_EG_TOKEN) == 0)        // Read positive example
      {
         ...
         posVertexIndices = AddVertexIndex(posVertexIndices,
                                           numPosExamples, vertexOffset);
         graph = posGraph;
         vertexListSize = &parameters->posGraphVertexListSize;
         edgeListSize = &parameters->posGraphEdgeListSize;
         startVertex = startPosVertex;
      ...
      else if (strcmp(token, NEG_EG_TOKEN) == 0)   // Read negative example
      {
         ...
         negVertexIndices = AddVertexIndex(negVertexIndices,
                                           numNegExamples, vertexOffset);
         graph = negGraph;
         vertexListSize = &parameters->negGraphVertexListSize;
         edgeListSize = &parameters->negGraphEdgeListSize;
      ...
      else if (strcmp(token, "v") == 0)         // read vertex
      {
         ...
         ReadIncrementVertex(graph, graphFile, labelList, vertexListSize,
                             &lineNo, vertexOffset);
      }
      else if (strcmp(token, "e") == 0)    // read 'e' edge
         ReadIncrementEdge(graph, graphFile, labelList, edgeListSize, &lineNo,
                           directed, startVertex, vertexOffset);
      else if (strcmp(token, "u") == 0)    // read undirected edge
         ReadIncrementEdge(graph, graphFile, labelList, edgeListSize, &lineNo,
                           FALSE, startVertex, vertexOffset);
      else if (strcmp(token, "d") == 0)    // read directed edge
         ReadIncrementEdge(graph, graphFile, labelList, edgeListSize, &lineNo,
                           TRUE, startVertex, vertexOffset);
      else
      {
         fclose(graphFile);
         FreeGraph(posGraph);
                         ...
                         ULONG *vertexListSize, ULONG *pLineNo, ULONG vertexOffset)
{
   ...
   sourceVertex = ReadInteger(fp, pLineNo);
   vertexID = sourceVertex + vertexOffset;
   ...
   AddVertex(graph, labelIndex, vertexListSize, sourceVertex);
   if (readingPositive)
                       ...
                       ULONG startVertexIndex, ULONG vertexOffset)
{
   ...
   AddEdge(graph, sourceVertexIndex, targetVertexIndex, directed, labelIndex,
           edgeListSize, spansIncrement);
   if (readingPositive)
