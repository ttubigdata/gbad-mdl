                       ...
                       Parameters *parameters)
{
   ...
   //
   // GBAD-P:  Variables for file containing output at each iteration
   //
   ...
   if (parameters->incremental)
   {
      ...
      for (v = 0; v < increment->numPosVertices; v++)
         if (!g1->vertices[vertexOffset + v].used)
         {
            ...
            //
            // GBAD-P:  Should only be the SUB_# vertex....
            //
            g2->vertices[vertexIndex].anomalous =
               g1->vertices[vertexOffset+v].anomalous;
            g2->vertices[vertexIndex].sourceVertex = 
	       g1->vertices[vertexOffset+v].sourceVertex;
            g2->vertices[vertexIndex].sourceExample = 0;  // would not be a
                                                          // source example for
                                                          // a compressed SUB
            //
	    // GBAD-P
            ...
            else
            {
               ...
               if (parameters->noAnomalyDetection)
                  ...
                  StoreAnomalousEdge(g2->edges, edgeIndex, v1, v2,
                                     ...
                                     g1->edges[edgeOffset + e].directed,
                                     g1->edges[edgeOffset + e].spansIncrement,
                                     g1->edges[edgeOffset + e].anomalous,
				     g1->edges[edgeOffset + e].sourceVertex1,
				     g1->edges[edgeOffset + e].sourceVertex2,
				     g1->edges[edgeOffset + e].sourceExample);
               //
   ...
   {
      //
      // GBAD-P:  Copy mapping to file.
      //
      if ((parameters->prob) || (parameters->mps)) {
         ...
         if (!(outputFile = fopen(outFileName, "w")))
	 {
      ...
      for (v = 0; v < g1->numVertices; v++)
         if (! g1->vertices[v].used) 
         {
            ...
            g2->vertices[vertexIndex].used = FALSE;
            //
            // GBAD-P:  Write vertex mapping and set anomalous flag.
            //
            if ((parameters->prob) || (parameters->mps))
