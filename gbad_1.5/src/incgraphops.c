...
//        (ULONG *vertexListSize) - pointer to the size of the vertex list
//
// RETURN:  void
//
...
//        (ULONG *edgeListSize) - pointer to the size of the edge list
                      ...
                      BOOLEAN spansIncrement)
{
   ...
   if (*edgeListSize == graph->numEdges)
   {
      *edgeListSize += LIST_SIZE_INC;
      newEdgeList =
         (ReferenceEdge *) realloc(graph->edges,
                                   (sizeof (ReferenceEdge) * (*edgeListSize)));
      if (newEdgeList == NULL)
...
RefInstanceListNode *AllocateRefInstanceListNode()
{
   RefInstanceListNode *refInstanceListNode =
      malloc(sizeof(RefInstanceListNode));

   refInstanceListNode->instanceList = AllocateInstanceList();
   refInstanceListNode->refGraph = NULL;
   refInstanceListNode->vertexListSize = 0;
   refInstanceListNode->edgeListSize = 0;
