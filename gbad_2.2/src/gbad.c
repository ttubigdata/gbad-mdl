         ...
         if (instanceList->head != NULL)
         {
            ...
            if (instanceListNode->instance != NULL)
            {
               if ((instanceListNode->instance->numVertices < sub->definition->numVertices) ||
                   (instanceListNode->instance->numEdges < sub->definition->numEdges))
               {
                  while (instanceListNode != NULL)
                  {
                     InstanceListInsert(instanceListNode->instance, parentInstanceList,
                                    ...
                                    Parameters *parameters)
{
   ...
   ULONG possibleEdgeChanges = g1->numEdges - (parameters->mdlThreshold * g1->numEdges) + 2;
   ULONG possibleVertexChanges = g1->numVertices - (parameters->mdlThreshold * g1->numVertices);
         ...
         for (v2 = 0; v2 < instance->numVertices; v2++)
         {
            ...
            for (e2 = 0; e2 < vertex2->numEdges; e2++)
            {
               ...
               if (! edge2->used)
               {
                  ...
                  // The idea here is to let it do some more extensions
                  // once it has close to the size expected...  however,
                  // what should that value be?  It works without this,
                  // although, we don't get exactly what we want....
