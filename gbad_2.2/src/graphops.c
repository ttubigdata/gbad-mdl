...
void ReadInputFile(Parameters *parameters)
{
   ...
   if (posGraph == NULL)
      ...
      parameters->originalPosGraph = CopyGraph(posGraph);
   parameters->originalLabelList = AllocateLabelList();
   for (index=0; index != labelList->numLabels; index++)
      StoreLabel(&(labelList->labels[index]), parameters->originalLabelList);
